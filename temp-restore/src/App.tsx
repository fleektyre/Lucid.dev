          ...
          <div>
            ...
            <ul className="space-y-4">
              {['Manifesto', 'Research', 'Dashboard'].map(item => (
                <li key={item}>
                  <button 
                    onClick={() => item === 'Dashboard' && onViewChange('generator')}
                    className="text-white/40 hover:text-white transition-colors text-sm font-light italic"
                  >
                    {item}
                  </button>
    ...
    <div className="min-h-screen bg-black flex flex-col items-center justify-center text-center p-6">
      ...
      {status === 'success' && (
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="space-y-8">
          ...
          <button onClick={() => onViewChange('generator')} className="bg-white text-black px-12 py-5 rounded-2xl text-[10px] font-bold uppercase tracking-widest">Return to Dashboard</button>
        </motion.div>
      )}
    </div>
  );
...
const LandingPage = ({ onViewChange, onStartBuilding }: { onViewChange: (v: any) => void; onStartBuilding: (p: string) => void }) => {
  return (
...
export default function App() {
  const [view, setView] = useState<'landing' | 'generator' | 'features' | 'auth' | 'payment-success'>('landing');
  ...
  // Sync view with URL for payment redirects
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-white/20 overflow-x-hidden">
      {view === 'landing' && <Navbar onViewChange={setView} currentView={view} />}
        ...
        <AnimatePresence mode="wait">
          {view === 'landing' && (
            <motion.div 
              ...
              transition={{ duration: 0.5 }}
            >
              <LandingPage onViewChange={setView} onStartBuilding={startBuilding} />
            </motion.div>
          )}
          ...
          {view === 'payment-success' && (
            <motion.div 
              ...
              exit={{ opacity: 0 }}
            >
              <PaymentSuccessPage onViewChange={setView} />
            </motion.div>
          )}
          ...
          {view === 'generator' && (
            <motion.div 
              ...
              transition={{ duration: 0.5 }}
            >
              <GeneratorPage initialPrompt={initialPrompt} onViewChange={setView} />
            </motion.div>
          )}
      ...
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Barlow:wght@300;400;500;600;700&family=Instrument+Serif:ital@0;1&family=Inter:wght@300;400;500;600;700&display=swap');
